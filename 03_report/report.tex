...
&= \mathbb{E}_q [\log \int \sum_{\mathbf{z}} \frac{p (\bm{\theta}, \mathbf{z}, \mathbf{w} | \bm{\alpha}, \bm{\beta})}{q(\bm{\theta}, \mathbf{z})}  d\bm{\theta}] \\
&\geq \mathbb{E}_q [\log p(\bm{\theta}, \mathbf{z}, \mathbf{w} | \bm{\alpha}, \bm{\beta})] + \mathbb{E}_q [\log q(\bm{\theta}, \mathbf{z})] \\
...
\begin{algorithmic}
   ...
   \ENDFOR
   \UNTIL{objective $\mathcal{L}$ doesn't improve anymore}
   \STATE $\theta_{d,k} = \frac{n_{d,k} + \bm{\alpha}_{k}}{\sum_{k'=1}^{K} n_{d,k'} + \bm{\alpha}_{k'}}$ for all $d$, $k$
   \STATE $\phi_{k,n} = \frac{n_{k,n} + \bm{\beta}_{n}}{\sum_{n'=1}^{N} n_{k,n'} + \bm{\beta}_{n'}}$ for all $k$, $n$
